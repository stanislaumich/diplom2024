...
DataModule1.QRep.Close;
        ...
        begin
                ...
                //  'tpl.xlsx');
                // https://delphisources.ru/pages/faq/base/excel_work.html
                XL.Workbooks[1].WorkSheets[1].Cells[7, 2] := Combobox6.Text;
                DataModule1.QTemp.Close;
                DataModule1.QTemp.SQL.Clear;
                DataModule1.QTemp.SQL.Add('select d.name name from dolg d, people p where p.iddolg=d.id and p.name='+Quotedstr(Combobox6.Text));
                DataModule1.QTemp.Open();
               // Memo1.Lines.add(DataModule1.QTemp.FieldByName('Name').AsString);
                XL.Workbooks[1].WorkSheets[1].Cells[6, 2] := DataModule1.QTemp.FieldByName('Name').AsString;
                for I := 0 to dbgrid14.DataSource.DataSet.RecordCount-1 do
                 begin
                 XL.Workbooks[1].WorkSheets[1].Cells[i+r, 1]:=inttostr(i+1);
...
DataModule1.QRep.Close;
        ...
        begin
                ...
                //  'tpl.xlsx');
                // https://delphisources.ru/pages/faq/base/excel_work.html
                XL.Workbooks[1].WorkSheets[1].Cells[6, 2] := Combobox8.Text;
                for I := 0 to dbgrid14.DataSource.DataSet.RecordCount-1 do
                 begin
                 XL.Workbooks[1].WorkSheets[1].Cells[i+r, 1]:=inttostr(i+1);
                  for j := 0 to dbgrid14.DataSource.DataSet.FieldCount-1 do
